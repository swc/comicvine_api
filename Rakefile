...
task :topypi => :test do
  cur_file = File.open("comicvine_api.py").read()
  comicvine_api_version = cur_file.scan(/__version__ = "(.*)"/)
  comicvine_api_version = comicvine_api_version[0][0].to_f

  puts "Build sdist and send comicvine_api v#{comicvine_api_version} to PyPi?"
  if $stdin.gets.chomp == "y"
    puts "Sending source-dist (sdist) to PyPi"

    if system("python setup.py sdist register upload")
      print "comicvine_api uploaded!"
    end
